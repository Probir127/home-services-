...
def admin_dashboard_stats(request):
    ...
    # Today's stats
    visitors_today = (
        Request.objects.filter(time__date=today).values("ip").distinct().count()
    )
    visitors_yesterday = (
        Request.objects.filter(time__date=yesterday).values("ip").distinct().count()
    )
    ...
    if visitors_yesterday > 0:
        visitors_change = int(
            ((visitors_today - visitors_yesterday) / visitors_yesterday) * 100
        )
    ...
    if quotes_yesterday > 0:
        quotes_change = int(
            ((quotes_today - quotes_yesterday) / quotes_yesterday) * 100
        )
    ...
    top_pages = (
        Request.objects.filter(time__gte=timezone.now() - timedelta(days=1))
        .values("path")
    ...
    top_pages_labels = [
        item["path"][:20] + "..." if len(item["path"]) > 20 else item["path"]
