...
def export_analytics_csv(request):
    ...
    for i in range(29, -1, -1):
        ...
        visitors = Request.objects.filter(time__date=date).values('ip').distinct().count()
...
def export_messages_csv(request):
    ...
    """
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = f'attachment; filename="messages_{timezone.now().strftime("%Y%m%d")}.csv"'
